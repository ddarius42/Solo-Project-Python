...
class Comment:
    ...
    @classmethod
    def getCommentWithPost(cls,data):
        query="SELECT * FROM comments LEFT JOIN posts on comments.commentId LEFT JOIN users ON comments.commentId WHERE comments.user_id = users.id AND comments.post_id = posts.id AND post_id = %(post_id)s;"
